...
int RTMetadataRetrieverCallback::fillVideoFrame(RtMetaData* meta) {
    ...
    ctx->mFrames.push_back(frameMem);
...
sp<IMemory> RTMetadataRetrieverCallback::allocVideoFrame(RtMetaData* meta) {
    ...
    frameCopy = static_cast<VideoFrame*>(frameMem->pointer());
...
int RTMetadataRetrieverCallback::extractFrames(std::vector<sp<IMemory> >* frames) {
    if (frames == NULL)
        return -1;
    ...
    for (size_t i = 0; i < ctx->mFrames.size(); i++) {
        frames->push_back(ctx->mFrames[i]);
    }
