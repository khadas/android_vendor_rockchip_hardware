...
status_t RockitMetadataRetriever::getFrameAtIndex(
...
status_t RockitMetadataRetriever::getFrameInternal(
    ...
    mCtx->mRtMetaData->setInt32(kRetrieverFrameNumber, numFrames);
    ...
    if ((outFrame == NULL) && (outFrames == NULL)) {
        ret = UNKNOWN_ERROR;
        goto _EXIT;
    }
    ...
    if (outFrame != NULL) {
        *outFrame = callBackCtx->extractFrame(NULL); // rect
        if (*outFrame != NULL) {
            ret = OK;
        }
    } else if (outFrames != NULL) {
        if (callBackCtx->extractFrames(outFrames) == 0) {
            ret = OK;
        } else {
            ret = UNKNOWN_ERROR;
        }
